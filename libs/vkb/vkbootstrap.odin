	...
	if self.minimum_instance_version > vk.API_VERSION_1_0 ||
	   self.required_api_version > vk.API_VERSION_1_0 {
		...
		if vk.EnumerateInstanceVersion == nil ||
		   ...
		   (self.minimum_instance_version == 0 && instance_version < self.required_api_version) { 	// Can't query version at all
...
physical_device_selector_is_device_suitable :: proc(
	...
	present_queue :=
		get_present_queue_index(pd.queue_families, pd.physical_device, self.instance.surface) !=
		vk.QUEUE_FAMILY_IGNORED
